...
| `/link_report` | ربط محادثة المشرف بالتقرير الأسبوعي بواسطة رمز لمرة واحدة |
| لوحة الويب | دخول مشرف، تقرير تفاعل مجمع، وإضافة وتعديل وحذف الأمراض والأسئلة |
| النشر اليومي | نشر واحد فقط يوميًا حتى عند استدعاء المجدول مرتين |
| التقرير الأسبوعي | ملخص كل أحد داخل لوحة الإدارة ورسالة Telegram إدارية |
...
ينشئ المخطط جداول المحتوى والاشتراك والنشر، إضافة إلى جداول العدادات اليومية المجمعة وإعداد توصيل التقرير. كما يفعّل **Row Level Security** ويلغي وصول `anon` و`authenticated`. لا تُنشئ سياسة عامة لهذه الجداول؛ التطبيق وحده يتصل من الخادم بمفتاح سري.
...
### ربط التقرير الأسبوعي بـTelegram

افتح **تقرير التفاعل** من شريط لوحة الإدارة، ثم اضغط **إنشاء رمز ربط Telegram**. يظهر أمر يبدأ بـ`/link_report` مرة واحدة ويكون صالحًا لعشر دقائق. افتح محادثة خاصة مع `@Intmed_edu_bot` وأرسل الأمر كاملًا. بعد ظهور رسالة نجاح الربط، ارجع إلى اللوحة واضغط **إرسال تقرير اختباري**.

يعرض التقرير إجمالي التفاعلات، ومشاهدات الموضوعات والتفاصيل والمراجع، وبدء الاختبارات وإجاباتها ودقتها، ونجاح البحث، وأكثر الموضوعات مشاهدة. لا يعرض مستخدمين فريدين لأن أحداث التحليل لا تخزن معرف Telegram أو الاسم أو نص الرسالة أو كلمات البحث أو الإجابات الفردية. تبدأ الإحصاءات من تاريخ تفعيل الميزة ولا تستعيد نشاطًا سابقًا.

في النشر الحالي، يستخدم النظام **Supabase Cron** مرتين يوميًا عند 05:00 و06:00 UTC لاستيعاب التوقيت الصيفي والشتوي في القدس. يتحقق Render من الساعة المحلية، وتمنع قيود قاعدة البيانات تكرار النشر أو التقرير. سر الاستدعاء مشفر في Supabase Vault ولا يظهر في Git. يوجد قالب إعادة الإعداد في `supabase/scheduler.sql.example`؛ لا تضع قيمة سر حقيقية داخل هذا الملف.
...
بعد الدخول تظهر بطاقات الأمراض والأسئلة والمشتركين. من **تقرير التفاعل** تراجع آخر 7 و30 يومًا، تربط مستلم Telegram، وترسل تقريرًا اختباريًا. من **الأمراض** يمكنك العرض والإضافة والتعديل والحذف. حذف مرض يحذف أسئلته المرتبطة. من **الأسئلة** تدير السؤال والخيارات والإجابة والشرح والصعوبة.
...
لا يستقبل النظام بيانات مرضى ولا يخزن محادثات المستخدمين. يحفظ معرف Telegram العددي الضروري لإرسال المنشور اليومي فقط، ويحفظ معرف محادثة إداريًا واحدًا لتوصيل التقرير، ولا يربط أيًا منهما بعدادات التفاعل. أحداث التحليل هي عدادات يومية مجمعة فقط. توجد مصفوفة الصلاحيات ومسار البيانات في [`SECURITY.md`](SECURITY.md) و[`docs/usage_analytics_spec.md`](docs/usage_analytics_spec.md). استخدم مفتاح Supabase السري في الخادم فقط؛ فهو يتجاوز RLS، ولذلك لا يجوز أن يظهر في HTML أو JavaScript أو Git.[1]
